...
const { buildCommentPrompt, buildAllStylesPrompt, buildOutreachPrompt, buildRecommendTonePrompt } = require('../prompts/commentPrompt');
...
/**
 * Runs the Outreach Skill for hiring/requirement posts.
 * Returns analysis + comment + 4 DM styles + 2 follow-ups.
 * Falls back to { isOutreach: false } if the post is not a hiring post.
 */
async function generateOutreach(params) {
  const { systemInstruction, userContent } = buildOutreachPrompt(params);
  return await callGeminiAPI(systemInstruction, userContent);
}
